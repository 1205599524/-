...
<div >
  ...
  <div class="add">
    ...
    <el-button type="primary" class="submit_btn"   @click="submit('databaseform')">修改</el-button>
...
export default {
   data(){
     ...
     return { database:{
       type:this.$parent.editdata.type,
       describe:this.$parent.editdata.describe,
       incode:this.$parent.editdata.incode,
        expend:this.$parent.editdata.expend,
        cash:this.$parent.editdata.cash,
        remark:this.$parent.editdata.remark,
        id:this.$parent.editdata.id
     },
     rules:{
       type:[{
         required:true,
   ...
   methods: {
      submit(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
           this.$axios.post(`/api/edit${this.database.id}`,this.database)
           .then(res=>{
             this.$message({
               message:"修改成功！",
               type:"success"
